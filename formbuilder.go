...
func New(templates *template.Template) *FormBuilder {
	return &FormBuilder{forms: make(map[string]*Form), templates: templates}
}
...
func (f *Form) Render(Values interface{}, Action, Method string) template.HTML {
	...
	if err := f.fb.templates.ExecuteTemplate(buf, "form.tpl", data); err != nil {
		fmt.Println(err)
	}
...
func (b *BoolField) Render(Values interface{}) template.HTML {
	...
	b.form.fb.templates.ExecuteTemplate(buf, "bool.tpl", data)
	return template.HTML(buf.String())
}
...
func (s *SubmitField) Render(Values interface{}) template.HTML {
	...
	s.form.fb.templates.ExecuteTemplate(buf, "submit.tpl", data)
	return template.HTML(buf.String())
}
